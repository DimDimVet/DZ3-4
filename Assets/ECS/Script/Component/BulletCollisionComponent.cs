...
public class BulletCollisionComponent : MonoBehaviour, IConvertGameObjectToEntity, IBulletCollisionComponent
{
    ...
    private void Start()
    {
        if (CollisionAction != null)//проверим на null лист скриптов
        {
            ...
            {
                if (CollisionAction[i] is IBulletCollisionComponent component)//проверим на наличие ICollisionsComponent лист скриптов
                {
                    ComponentCollisions.Add(component);//добавим в лист ICollisionsComponent найденое
                }
                else
                {
                    Debug.Log("Нет компонентов с IBulletCollisionComponent");
                }
            }
        }
